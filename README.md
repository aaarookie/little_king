...
- **Sprint 2**：卡牌图标与精灵、法术门锁定 UI、调试命令（CheatManager）、DT_Waves 填表
- **Sprint 3**：英雄技能（GAS Ability + 冷却）、英雄血条 UI、胜负手感
- **Sprint 4**：敌方 AI 升级（集火/反制）、内容扩充与首次平衡
- **阶段 2~6**：地牢局内循环 → 家园局外循环 → 内容扩充 → 美术音乐替换 → 发布

## ⚠️ 已知事项

- 美术/音乐为占位（调试色块 + 暂无音效），后续按 AI 生成流程替换
