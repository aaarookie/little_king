...
- [ ] D0：规则基线、状态所有权和战斗接入契约。
- [ ] D1：固定三战串联，先打通选节点、进战斗、结算返回、下一战。
- [ ] D2：数据驱动遭遇、独立双方阵容、确认后的跨房间英雄状态。
- [ ] D3：三选一奖励、跳过、现有卡升级与唯一卡组，防重复领取。
- [ ] D4：3～5 场分支路线、精英/休息/首领节点与种子复现。
- [ ] D5：安全节点存档、继续/放弃、远征总结与功能收口。
